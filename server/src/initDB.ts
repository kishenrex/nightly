...
const initDB = async () => {
    ...
    try {
        ...
        if (tableExists) {
            ...
            await db.exec(`
                INSERT INTO users (email, username, password, avatar, current_streak, max_streak, created_at, updated_at)
                SELECT email, username, password, avatar, current_streak, max_streak, created_at, updated_at
                FROM users_old;
            `);
